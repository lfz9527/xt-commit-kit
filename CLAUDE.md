...
6. **package.json 修改** — 设置 `scripts.prepare`，移除已弃用的 `husky.hooks`，合并 `lint-staged` 配置，并设置 `config.commitizen` + `config.cz-customizable`。通过点路径解析处理嵌套键。
...
| `package.json` → `lint-staged` | `src/configs/lint-staged.config.json` | 默认匹配模式：`*.{js,jsx,ts,tsx}` 运行 `eslint --fix` + `prettier --write`；CSS/JSON/MD 等仅运行 `prettier --write` |
